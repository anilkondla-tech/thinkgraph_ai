...
import { computeUserCookieKey } from "@/lib/userSites";
...
export async function POST(req: Request) {
  ...
  const COOKIE = computeUserCookieKey(session.user.email);
  ...
  // Read this user's existing sites from their personal cookie
  ...
  // Store encrypted in this user's personal HttpOnly cookie
  cookieStore.set(COOKIE, encrypt(JSON.stringify(updated)), COOKIE_OPTIONS);
...
/** Remove a workspace by key for the authenticated user. */
